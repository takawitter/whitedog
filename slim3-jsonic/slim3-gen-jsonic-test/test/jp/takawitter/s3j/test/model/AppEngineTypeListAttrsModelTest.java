...
public class AppEngineTypeListAttrsModelTest {
	...
	public void jsonToModel() throws Exception{
		AppEngineTypeListAttrsModel m = AppEngineTypeListAttrsModelMeta.get().jsonToModel(
				"{\"blobKeyListAttr\":[\"lkwejl2k3jrksl\",\"kaekl23joij\"]" +
				",\"blobListAttr\":[\"aGVsbG8=\",\"d29ybGQ=\"]" +
				",\"categoryListAttr\":[\"partOfSpeech\",\"kind\"]" +
				",\"emailListAttr\":[\"a@b.com\",\"d@e.com\"]" +
		...
		Assert.assertArrayEquals(
				Arrays.asList(new Email("a@b.com"), new Email("d@e.com")).toArray()
				, m.getEmailListAttr().toArray());
		Assert.assertArrayEquals(
				Arrays.asList(new BlobKey("lkwejl2k3jrksl"), new BlobKey("kaekl23joij")).toArray()
				, m.getBlobKeyListAttr().toArray());
		Assert.assertArrayEquals(
				Arrays.asList(new Blob("hello".getBytes()), new Blob("world".getBytes())).toArray()
				, m.getBlobListAttr().toArray());
		Assert.assertArrayEquals(
				Arrays.asList(new Category("partOfSpeech"), new Category("kind")).toArray()
				, m.getCategoryListAttr().toArray());
		Assert.assertArrayEquals(
				Arrays.asList(new GeoPt(1.0f, 2.0f), new GeoPt(3.0f, 4.0f)).toArray()
		...
		Assert.assertArrayEquals(
				Arrays.asList(new Rating(80), new Rating(90)).toArray()
				, m.getRatingListAttr().toArray());
		Assert.assertArrayEquals(
				Arrays.asList(
						new ShortBlob("hello".getBytes()), new ShortBlob("world".getBytes())
						).toArray()
